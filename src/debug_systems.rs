use crate::boids::{Boid, BoidsAlignment, BoidsBoundaries, BoidsCoherence, BoidsRules, BoidsSeparation, GameRules};
use bevy::math::vec3;
use bevy::prelude::*;
use bevy::render::camera::RenderTarget;
use bevy_inspector_egui::{Inspectable, InspectorPlugin};
use bevy_prototype_debug_lines::DebugLines;
use bevy_rapier2d::prelude::{RapierContext, Sensor};
...
pub fn debug_world_bounds(
    query: Query<(&Transform, &BoidsBoundaries)>,
    ...
    for (tf, bound) in query.iter() {
        lines.line_colored(
            tf.translation,
            tf.translation + bound.force,
            duration,
            Color::CYAN,
        );
    }
...
pub fn debug_cohesion(
    query: Query<(&Transform, &BoidsCoherence, &DebugBoid)>,
    ...
    for (tf, coh, debug) in query.iter() {
        ...
        lines.line_colored(
            tf.translation,
            tf.translation + coh.force / rules.coherence_factor,
            0.0,
            Color::GREEN,
        );
    }
...
pub fn debug_separation(
    mut boids: Query<(&Transform, &mut BoidsSeparation)>,
    ...
    for (sensor_ent, _, parent, gtf) in sensors.iter() {
        ...
        if let Ok((tf, mut sep)) = boids.get_mut(parent.get()) {
            match count {
                ...
                _ => {
                    ...
                    lines.line_colored(
                        tf.translation,
                        tf.translation + vec / rules.separation_factor,
                        0.0,
                        Color::RED,
                    );
                }
...
fn debug_alignment(
    query: Query<(&Transform, &BoidsAlignment, &DebugBoid)>,
    rules: Res<BoidsRules>,
    config: Res<DebugConfig>,
    mut lines: ResMut<DebugLines>,
) {
    ...
    for (tf, ali, debug) in query.iter() {
        lines.line_colored(
            tf.translation,
            tf.translation + ali.force / rules.alignment_factor,
            0.0,
            Color::GREEN,
        );
    }
